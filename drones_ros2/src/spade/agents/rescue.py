...
class RobotPositionSubscriber(Node):
    def __init__(self, namespace, agent):
        ...
        self.agent = agent  # Store the reference to the RescueAgent
    ...
    def robot_pose_callback(self, msg):
        ...
        if self.calculate_distance(self.position, self.station_poses[self.agent.id]) < 3.0:
            # print(f"Back at station {self.namespace}")
            self.at_station = True
            self.agent.update_at_station_status(True)
        else:
            self.at_station = False
            self.agent.update_at_station_status(False)
    ...
    async def navigate_to_human(self, agent_id, drone_id, target, agent):
        # if drone_id == agent_id:
        print(colored(f"Rescue robot {agent_id} is navigating to human detected by drone {drone_id}!",'yellow'))
...
class RescueAgent(Agent):
                ...
                if self.agent.robot_subscriber.position[0] != 0 and self.agent.robot_subscriber.position[1] != 0:
                    if robot_type == "drone":
                        ...
                        if not other_at_station:
                            print(f"Info from robot {self.agent.id}: other robot is busy")
                        else:
                             print(f"Info from robot {self.agent.id}: distance to target: {other_robot_distance}")
        ...
        async def run(self):
            ...
            if self.agent.distances:
                ...
                for drone_id, distance in self.agent.distances.items():
                    msg = Message(
                        to=self.agent.receiver_another,
                        body=str(("robot", drone_id, distance, self.agent.robot_subscriber.at_station))
                    )
                    messages_to_send.append(msg)
                    # print(str(("robot", drone_id, distance, self.agent.robot_subscriber.at_station)))
    ...
    async def setup(self):
        ...
        self.robot_subscriber = RobotPositionSubscriber(self.namespace, self)  # Pass self here
