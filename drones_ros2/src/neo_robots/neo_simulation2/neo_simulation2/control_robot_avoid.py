...
class GroundRobotController(Node):
    ...
    def scan_callback(self, msg):
            ...
            if self.current_pose is not None:
                if min_range < 1.2 or self.state == 'TURN_AWAY_FROM_OBSTACLE':
                    ...
                    # Check if turned less than 20 degrees
                    if turned_amount < math.radians(50):
                        vel_msg.angular.z = 0.5 * self.calculate_turn_direction(desired_yaw)
                    else:
                        # Stop turning once turned more than 20 degrees
                    ...
                    if self.state == 'TURN_AWAY_FROM_OBSTACLE':
                        self.state = 'MOVE_FORWARD_AFTER_AVOIDANCE'
                        self.forward_start_position = self.current_pose.position
    ...
    def calculate_turn_direction(self, desired_yaw):
        current_yaw = self.get_yaw(self.current_pose.orientation)
        difference = self.normalize_angle(desired_yaw - current_yaw)
