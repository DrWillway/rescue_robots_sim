...
class DroneObject(Node):
    def __init__(self, node_name: str = "drone_node", namespace = "/drone/"):
        ...
        # Publishers
        self.pubTakeOff = self.create_publisher(Empty, namespace + 'takeoff', 1024)
        self.pubLand = self.create_publisher(Empty, namespace + 'land', 1024)
        self.pubReset = self.create_publisher(Empty, namespace + 'reset', 1024)
        self.pubPosCtrl = self.create_publisher(Bool, namespace + 'posctrl', 1024)
        self.pubCmd = self.create_publisher(Twist, namespace + 'cmd_vel', 1024)
        self.pubVelMode = self.create_publisher(Bool, namespace + 'dronevel_mode', 1024)

        # Subscribers
        self.sub_sonar = self.create_subscription(Range, namespace + 'sonar', self.cb_sonar, 1024)
        self.sub_imu = self.create_subscription(Range, namespace + 'imu', self.cb_imu, 1024)
        self.sub_front_img = self.create_subscription(Image, namespace + 'front/image_raw',
                                                      self.cb_front_img, 1024)
        self.sub_bottom_img = self.create_subscription(Image, namespace + 'bottom/image_raw',
                                                       self.cb_bottom_img, 1024)
        self.sub_gt_pose = self.create_subscription(Pose, namespace + 'gt_pose', self.cb_gt_pose, 1024)
        self.sub_state = self.create_subscription(Int8, namespace + 'state', self.cb_state, 1024)
        self.sub_cmd_mode = self.create_subscription(String, namespace + 'cmd_mode', self.cb_cmd_mode, 1024)
